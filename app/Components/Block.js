...
export default function  Block ({posx,posy,visible,index,width,height,gameId,blockId,handleVisibility,increaseSize,decreaseSize,SetPosition})  {
    ...
    useEffect(()=>{
      elemRef.current.style.left = `${posx}px`
      elemRef.current.style.top =`${posy}px`
    },[])
    ...
    const stopDragging = ( ) => {
      ...
      SetPosition(index-1,x+window.scrollX,y+window.scrollY)
    }
    ...
    const saveBlock = () =>{
      ...
      fetch("/api/blocks",{
        ...
        .then(n => console.log("Save Block"))
        .catch(n => console.error("Could not sync game"))
    }
    ...
    return (
      <div
        ...
        ref={elemRef}
        className={` bg-slate-950 rounded-lg  ${visible?'bg-opacity-100':'bg-opacity-0'} flex flex-col justify-center items-center p-5`}
        style={{
            position:"absolute",
            width:`${width}vw`,
            height:`${height}vw`,
        ...
        >
            ...
            <div className='flex flex-row w-full justify-between items center rounded-lg'>
                    <button className='opacity-20 w-1/3 text-slate-600  py-2 px-4 mt-3 font-bold border-1 border-slate-600 p-5 hover:bg-slate-900 hover:text-slate-200' onClick={()=>increaseSize(index-1)}>
                    ...
                    <button className='opacity-20 w-1/3 text-slate-600  py-2 px-4 mt-3  font-bold border-1 border-slate-600 p-5 hover:bg-slate-900 hover:text-slate-200' onClick={()=>handleVisibility(index-1)}>
                    {visible?
                    ...
                    <button className='opacity-20 w-1/3 text-slate-600  py-2 px-4 mt-3 font-bold border-1 border-slate-600 p-5 hover:bg-slate-900 hover:text-slate-200' onClick={()=>decreaseSize(index-1)}>
            ...
            <div className='absolute bottom-4 right-4 flex flex-row justify-center items-center'>
              <sub className=' text-slate-200'>{String.fromCharCode(96 + index)}</sub>    
            </div>
